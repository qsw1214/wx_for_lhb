    ...
    <div class="drop">
     ...
     <div class="drop">
      ...
      <div class="type-box">
          ...
          @click.stop="districtSelect( item ,index)"
           :key="index"
           >
              <span>{{ item.name }}</span>
          </div>
...
export default {
  data () {
    return {
      ...
      lowPrice : null ,// 最低价
      highPrice : null ,
      minimum_peoples : null , // 人流量最小
      maximum_peoples : null ,
      minimum_build_year : null , // 建筑年份最小值
      maximum_build_year : null ,
      minimum_households : null , // 户数最小值
      maximum_households : null ,
      minimum_property_costs : null , // 物业费的最小数
      maximum_property_costs : null ,
      minimum_house_price : null , // 房价的最小数
      maximum_house_price : null,
      minimum_area : null , // 最小面积
    }
  },
  computed : {
    ...mapState(['filterData']),
  ...
  methods:{
    ...
    changeSelectData(obj){
      for (let item of obj) {
        Object.assign(item , { "is_select": 0 });
      }
      return obj;
    },
    ...
    submit(){
      ...
      for( let item of this.fieldtypes ){
        if( item.is_select ){
          this.field_type_id.push( item.field_type_id )
        }
      }
      ...
      if( this.field_type_id.length ){
        this.field_type_id   = [...new Set( this.field_type_id )] ;
        this.childFilterData.field_type_id = this.field_type_id
      }
    ...
    showSelected(){
      ...
      if( JSON.stringify(this.filterData) != "{}" ){
        ...
        if( this.filterData.field_type_id&&this.filterData.field_type_id.length ){
          for( let item of this.filterData.field_type_id ){
            for( let i of this.fieldtypes ){
              if( i.field_type_id == item ){
                i.is_select = true ;
              }
            }
          }
