<template>
  <div>
    ...
    <div class="wrapBox" ref="swiper">
      <swiper loop dots-position="center" dots-class="custom-bottom" :aspect-ratio="600/750">
        <swiper-item v-for="( item , index ) in fieldImgs" :key="index">
          <div class="imgbox">
            <img v-lazy="item.pic_url" :data-srcset="item.pic_url | large" alt="场地图片" @click.stop="show(index)" class="previewer-demo-img">
          </div>
        </swiper-item>
      </swiper>
      <div class="typeposition" v-if="fieldsdetails">{{ fieldsdetails.field_type.display_name }}</div>
    </div>
    ...
    <section class="res-detail" ref="resDetail">
      ...
      <div class="gutter">
        <router-link :to="{ name : 'fieldinfo' }" class="view-all"><span class="view-text">查看所有信息</span><span class="cert-icon"></span></router-link>
      </div>
    </section>
      ...
      <div class="list-wraper">
          ...
          <ul class="list" :style="{ width : otherResource.length*4.48 + 'rem' }">
            <li class="res-item" v-for="(item,index) in otherResource" :key="index">
              <router-link :to="{ name : 'fieldsDetail' , params : { id : item.id } }">
                <div class="img-box">
                  <img v-lazy="item.pic_url" :data-srcset="item.pic_url | half" alt="资源图片">
                  <!--<div class="date">2017.07.31-2017.05.14</div>-->
                </div>
                <div class="text-info">
    ...
    <section class="recommend" v-if="list.length">
      ...
      <div class="rec-list" v-load-more="loadingMore" type="1">
        <div class="rec-item" v-for="(item ,index) in list" :key="index">
          <router-link :to="{ name : 'fieldsDetail' , params : { id : item.resource_id } }" class="rec-link">
            <div class="img-box">
              <img v-lazy="item.pic_url" :data-srcset="item.pic_url | half" alt="a">
              <div class="position">{{ item.field_type }}</div>
            </div>
            <div class="rec-info">
              ...
              <div class="price-people">
                ...
                <div class="people">
                  <span>人流量：</span><span>{{ item.number_of_people }}</span>
                </div>
              </div>
            </div>
          </router-link>
...
import { getResourcesDetail , getOtherResources , getResourcesComments , getRecommendingResources , removeFavorite , addFavorite , addCartList ,getCartCount} from 'src/service/getData';
...
import { half , twoPointPre , twoPointNext , large , hundred } from 'src/filters';
export default {
  directives: {
    TransferDom
  },
  ...
  mounted() {
    ...
    getResourcesDetail( id , params ).then( res => {
      ...
      let other = { res_type_id : 1 , current_resource_id : id , page : 1} ; // 场地资源
  ...
  filters :{
    large,half,twoPointPre,twoPointNext ,hundred
  },
  methods : {
    drop(des){
