  ...
  <div class="field-wraper">
   <div class="fixed-wraper">
    <header class="border-1px">
      ...
      <div class="search vt">
        <router-link :to="{ path : '/search' , query : { resourceType : 3 }}">
    ...
    <section class="filter">
       <flexbox :gutter="0">
          <flexbox-item :span="1/4" class="ct">
              <div class="filter-rule" :class="{
                 active : filterData.order_by == 'created_at'|| filterData.order_by == 'number_of_people'|| filterData.order_by =='cost_performance' }" @click.stop="listSelected">
          ...
          <flexbox-item :span="1/4" class="ct">
              <div class="filter-btn" :class="{ active : filterSelect }" @click.stop="filterSelected">
                <router-link to="/activities/acfilter">
       ...
       <div class="dropdown" v-show="showDropdown" ref="mask" @click.stop="showDropdown = false">
          <ul class="down-ul">
            <li class="down-ul-item" v-for="( item , index ) in fieldSort" v-if="!item.default_sort" :class="{ active : filterData.order_by == item.order_by && filterData.order == item.order }" @click="pickPx( item , index )" :key="index">
    ...
    <section class="list" v-load-more="loadingMore" type="1" v-if="total>0">
        <!-- 记录搜索总数 -->
        <v-activitylist :field="item" v-for="( item , index ) in fields" :key="index"></v-activitylist>
        <load-more v-show="is_show" :show-loading="showLoading" :tip="loadingTip" background-color="#e8e8e8"></load-more>
    </section>
    <section class="noresult-box" v-else ref="noresult">
...
<script>
  ...
  import vActivitylist from 'src/components/vActivitylist';
  ...
  let actChildData = null ; // 记录筛选框里筛选条件
export default {
  data(){
    return {
      fields : [], // 场地列表
  ...
  components: { Flexbox , FlexboxItem , vActivitylist , LoadMore },
  mixins : [ loadMore ] ,
...
// 处理从搜索页或者app传过来的筛选条件
  ...
  mounted(){
    ...
    if( JSON.stringify( actChildData ) == "{}"|| JSON.stringify( actChildData ) == "null" ){
      // 空值，不高亮
      this.filterSelect = false ;
    }else{
      this.fields = [] ;
      this.filterSelect = true ;
      this.filterData = { ...this.filterData , ...actChildData } ;
    }
    this.init();
    // 渲染综合排序的数据
  ...
  methods:{
    ...
    marge( data ){
      ...
      if( actChildData ){
        // 有值 删除上一次的筛选条件
        for(let item in actChildData ){
          delete this.filterData[item] ;
        }
      }
      actChildData = Object.assign( {} , data ) ;
